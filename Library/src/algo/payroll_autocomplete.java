...
public class payroll_autocomplete {

    public payroll_autocomplete(JTextField txtInput, String decider) throws Exception {
       ...
       if(decider.matches("name")) {
        for (int i = 0; rs.next(); i++) {
            String item =rs.getString("Employee_name");
            items.add(item);
        }
       }
       else if(decider.matches("id")) {
           for (int i = 0; rs.next(); i++) {
               String item =rs.getString("Employee_id");
               items.add(item);
           }
          }
