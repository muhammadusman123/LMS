...
public class purchase_autocomplete {

	public purchase_autocomplete(JTextField txtInput, String decider)
			throws Exception {
		...
		if (decider.matches("id")) {
			...
			for (int i = 0; rs.next(); i++) {
				String item = rs.getString("ISBN");
				items.add(item);
			}
		} else if (decider.matches("author")) {
			rs = obj.queryrep();
			for (int i = 0; rs.next(); i++) {
				String item = rs.getString("Author_Name");
				items.add(item);
			}
		} else if (decider.matches("book")) {
			rs = obj.queryrep();
			for (int i = 0; rs.next(); i++) {
			...
			rs = obj.queryrep();
			for (int i = 0; rs.next(); i++) {
				String item = rs.getString("Purchase_Date");
				items.add(item);
			}
		}
