...
public class issueautocomplete {

	public issueautocomplete(JTextField txtInput, String decider)
			throws Exception {
		...
		if (decider.matches("id")) {
			for (int i = 0; rs.next(); i++) {
				String item = rs.getString("issuanceid");
				items.add(item);
			}
		...
		} else if (decider.matches("issuedate")) {
			rs = obj.queryrep();
			for (int i = 0; rs.next(); i++) {
				String item = rs.getString("issuedate");
				items.add(item);
			}
		} else if (decider.matches("returndate")) {
			rs = obj.queryrep();
			for (int i = 0; rs.next(); i++) {
				String item = rs.getString("returndate");
				items.add(item);
