...
public class returnautocomplete {

	public returnautocomplete(JTextField txtInput, String decider)
			throws Exception {

		returndb obj = new returndb();
		ResultSet rs = obj.queryrep();
		ArrayList<String> items = new ArrayList<String>();
		if (decider.matches("returnid")) {
			for (int i = 0; rs.next(); i++) {
				String item = rs.getString("returnid");
				items.add(item);
			}
		} else if (decider.matches("issueid")) {
			rs = obj.queryrep();
			for (int i = 0; rs.next(); i++) {
				String item = rs.getString("issueid");
				items.add(item);
			}
